...
Text::Markup::Parser - Core class of the markup parser.

=head1 SYNOPSIS

When configured with a Tokenizer instance, this class converts a Text::Markup token
stream into a tree structure similar in nature to a DOM.

=over

my $parse=Text::Markup::Parse->new(
    tokenizer => Text::Markup::Tokenizer->new());

my $tree=$parse->parse($source);

=back

A parsed tree as generated by the above will contain a Text::Markup::Tree node named
body as the root node.  The body member of each node consists of an array of text
scalars, which should be treated as raw text, and Text::Markup::Tree objects.  Refer to
Text::Markup::Tree for a more detailed description of the relevant fields.
...
sub parse {
    ...
    return $self->_parse_internal(Text::Markup::Tree->new(), \@tokens);
}
